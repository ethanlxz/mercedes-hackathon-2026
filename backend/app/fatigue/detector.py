...
MPL_CONFIG_DIR = Path(__file__).resolve().parents[3] / ".matplotlib-cache"
MPL_CONFIG_DIR.mkdir(exist_ok=True)
os.environ.setdefault("MPLCONFIGDIR", str(MPL_CONFIG_DIR))

import mediapipe as mp

from backend.app.fatigue import config
from backend.app.fatigue.utils import (
    collect_points,
    eye_aspect_ratio,
    mouth_open_ratio,
    resize_frame,
    ...
    def _track_head_nod(self, nose_y: float, now: float) -> bool:
        ...
        if self.head_nod_peak_y is not None:
            ...
            returned_near_baseline = (
                abs(nose_y - self.nose_y_baseline)
                <= config.HEAD_NOD_REBOUND_THRESHOLD
            )
